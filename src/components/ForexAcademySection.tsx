...
import { ForexPatternCheatSheet } from './ForexPatternCheatSheet';
import { ForexSessionAndChecklist } from './ForexSessionAndChecklist';
...
export const ForexAcademySection: React.FC<ForexAcademySectionProps> = ({
  ...
  const forexArticles = articles.filter(
    (a) => a.category === 'forex' || a.tags.some((t) => t.toLowerCase().includes('forex') || t.toLowerCase().includes('fedha'))
  );
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-16">
      {/* Top Breadcrumb & Subpage Title Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          ...
          <h1 className="display-serif text-2xl sm:text-4xl font-extrabold text-white tracking-tight mt-1">
            Chuo cha Forex & Uchambuzi wa Masoko ya Kimataifa
          </h1>
          <p className="text-xs sm:text-sm text-zinc-400 mt-1 max-w-3xl">
            Jifunze biashara ya sarafu (Forex Trading) kuanzia misingi ya pips na lot sizes, Candlestick Trading Bible, Smart Money Concepts (SMC), hadi usimamizi wa mtaji kwa lugha adhimu ya Kiswahili.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-white/10 pb-3">
        <button
          onClick={() => { setActiveTab('modules'); stopSpeech(); }}
        ...
        >
          ...
          <span>Mtaala Kamili ({FOREX_MODULES.length} Moduli)</span>
        </button>

        <button
          onClick={() => { setActiveTab('patterns'); stopSpeech(); }}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'patterns'
              ? 'bg-[#10b981] text-black shadow-lg shadow-emerald-500/20'
              : 'bg-zinc-900 text-zinc-400 hover:text-white hover:bg-zinc-800'
        ...
        >
          ...
          <span>Atlas ya Candlestick & SMC</span>
        </button>

        <button
          onClick={() => { setActiveTab('sessions'); stopSpeech(); }}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'sessions'
              ? 'bg-[#10b981] text-black shadow-lg shadow-emerald-500/20'
              : 'bg-zinc-900 text-zinc-400 hover:text-white hover:bg-zinc-800'
        ...
        >
          ...
          <span>Vipindi vya Masoko & Safety Checklist</span>
        </button>

        <button
          onClick={() => { setActiveTab('calculator'); stopSpeech(); }}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
        ...
        >
          ...
          <span>Kikokotoo cha Lot Size & Risk</span>
        </button>

        {forexArticles.length > 0 && (
          <button
            onClick={() => { setActiveTab('articles'); stopSpeech(); }}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
              activeTab === 'articles'
                ? 'bg-[#10b981] text-black shadow-lg shadow-emerald-500/20'
                : 'bg-zinc-900 text-zinc-400 hover:text-white hover:bg-zinc-800'
          ...
          >
            ...
            <span>Makala za Forex ({forexArticles.length})</span>
          </button>
        )}
      </div>

      {/* VIEW 1: Full 7 Educational Modules View */}
      {activeTab === 'modules' && (
              ...
              <div className="bg-zinc-900/90 border border-white/10 rounded-2xl p-4 sticky top-24 space-y-3">
                ...
                <div className="pt-3 border-t border-white/10">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full bg-zinc-950 hover:bg-zinc-800 text-emerald-400 border border-emerald-500/30 font-bold text-xs py-2.5 rounded-xl transition-colors cursor-pointer"
                  >
                    <Calculator className="w-4 h-4" />
      ...
      {/* VIEW 3: Vipindi vya Masoko & Safety Checklist */}
      {activeTab === 'sessions' && (
        <div className="space-y-6">
          <ForexSessionAndChecklist />
        </div>
      )}

      {/* VIEW 4: Kikokotoo cha Lot Size & Usimamizi wa Hatari */}
      {activeTab === 'calculator' && (
        <div className="space-y-6">
          <ForexCalculatorTool />
        </div>
      ...
      {/* VIEW 5: Makala za Forex */}
      {activeTab === 'articles' && (
        <div className="space-y-6">
          <div className="bg-zinc-900/80 border border-white/10 rounded-2xl p-6">
            <h3 className="text-xl font-black text-white mb-2">Makala na Uchambuzi wa Forex</h3>
            <p className="text-xs text-zinc-400">
              Soma makala za kina za kiuchumi, uchambuzi wa mwelekeo wa Shilingi ya Tanzania (USD/TZS), na mikakati ya kupita majaribio ya Prop Firms.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {forexArticles.map((art) => (
              <div
                key={art.id}
                onClick={() => onSelectArticle && onSelectArticle(art)}
                className="bg-zinc-900/90 border border-white/10 hover:border-emerald-500/40 rounded-2xl p-5 space-y-3 cursor-pointer transition-all hover:-translate-y-1 group"
              >
                {art.imageUrl && (
                  <img
                    src={art.imageUrl}
                    alt={art.title}
                    className="w-full h-44 object-cover rounded-xl border border-white/5 group-hover:opacity-90 transition-opacity"
                  />
                )}
                <div className="flex items-center justify-between text-[11px] text-zinc-400">
                  <span className="bg-emerald-500/20 text-emerald-400 font-bold px-2 py-0.5 rounded uppercase">
                    {art.categoryName || 'FOREX'}
                  </span>
                  <span>{art.publishedAt}</span>
                </div>
                <h4 className="text-base font-bold text-white group-hover:text-[#10b981] transition-colors line-clamp-2">
                  {art.title}
                </h4>
                <p className="text-xs text-zinc-400 line-clamp-3 leading-relaxed">
                  {art.excerpt}
                </p>
                <div className="pt-2 border-t border-white/5 flex items-center justify-between text-xs text-[#10b981] font-bold">
                  <span>Soma Makala Kamili</span>
